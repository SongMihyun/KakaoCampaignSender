...
from frontend.pages.contacts.tabular_editor_dialog import WordEditorDialog, TextEditorDialog
...
from backend.integrations.excel.contacts_exporter import (
...
class ContactsPage(QWidget):
    ...
    def _edit(self) -> None:
        ...
        ok = edit_contact_by_id(self, contacts_service=self.service, contact_id=int(preset.id))
        if not ok:
            return
    ...
    def _delete_checked(self) -> None:
        ...
        if not ids:
            ...
            return
        ok = QMessageBox.question(self, "삭제 확인", f"{len(ids)}건을 삭제하시겠습니까?", QMessageBox.Yes | QMessageBox.No)
    ...
    def _pick_import_file(self, mode: str = "all") -> None:
        try:
            if mode == "excel":
                ...
                default_ext = "xlsx"
            path = pick_open_file(title="대상자 파일 선택", filters=filters, default_ext=default_ext)
        except Exception as e:
    ...
    def _import_file_from_path(self, path: str, source_label: str) -> None:
        ...
        run_bg(parse_job, on_done=lambda result: self._after_parse(result, source_label=source_label), on_error=self._show_bg_error)
    ...
    def _download_template(self, kind: str = "excel") -> None:
        try:
            if kind == "word":
                filters = [Filter("Word Files", "*.docx"), Filter("All Files", "*.*")]
                default_filename = "contacts_template.docx"
                default_ext = "docx"
            elif kind == "text":
                filters = [Filter("Text Files", "*.txt"), Filter("All Files", "*.*")]
                default_filename = "contacts_template.txt"
                default_ext = "txt"
            else:
                filters = [Filter("Excel Files", "*.xlsx"), Filter("All Files", "*.*")]
                default_filename = "contacts_template.xlsx"
                default_ext = "xlsx"
            path = pick_save_file(title="샘플 서식 저장", filters=filters, default_filename=default_filename, default_ext=default_ext)
        except Exception as e:
        ...
        try:
            if kind == "word":
                if not path.lower().endswith(".docx"):
                    path += ".docx"
                create_template_docx(path)
            elif kind == "text":
                if not path.lower().endswith(".txt"):
                    path += ".txt"
                create_template_txt(path)
            else:
    ...
    def _export_contacts(self, kind: str = "excel") -> None:
        try:
            if kind == "word":
                filters = [Filter("Word Files", "*.docx"), Filter("All Files", "*.*")]
                default_filename = "contacts_export.docx"
                default_ext = "docx"
            elif kind == "text":
                filters = [Filter("Text Files", "*.txt"), Filter("All Files", "*.*")]
                ...
                default_ext = "xlsx"
            path = pick_save_file(title="대상자 내보내기", filters=filters, default_filename=default_filename, default_ext=default_ext)
        except Exception as e:
            QMessageBox.critical(self, "오류", f"저장 위치 선택기 실행 실패:\n{e}")
            return
        if not path:
            return
        if kind == "word" and not path.lower().endswith(".docx"):
            path += ".docx"
        ...
        def done(cnt: int):
            ...
            QMessageBox.information(self, "완료", f"내보내기 완료: {cnt}건")
    ...
    def _on_contact_double_clicked(self, index) -> None:
        ...
        if not src.isValid():
            return
        c = self.model.contact_at(src.row())
        ok = edit_contact_by_id(self, contacts_service=self.service, contact_id=int(c.id))
        if not ok:
            return
