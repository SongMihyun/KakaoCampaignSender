...
from kakao_pc_driver.hooks import open_chat_by_name_hook, send_image_dialog_hook, ChatNotFound, _is_kakao_toast_quick_reply
from kakao_pc_driver.image_attach_ctrl_t import (
    send_png_via_ctrl_t,
    send_files_via_ctrl_t,
)
from ctypes import wintypes
from kakao_pc_driver.image_attach_ctrl_v import attach_image_via_ctrl_v
...
from kakao_win32 import win32_core as w32
...
class KakaoPcDriver(KakaoSenderDriver):
    ...
    def _paste_image_and_send(self, png_bytes: bytes) -> bool:
        ...
        # ✅ 클립보드 붙여넣기 사이사이에 무작위로 Ctrl+T 파일첨부를 섞는다.
        #    (전송 방식 자체가 벤을 피해준다는 보장은 없지만, 사용자 요청으로
        #    "발송 방식에 변수를 준다"는 원칙에 맞춰 되살림)
        #    확률 기반이라 "정확히 4번에 1번"처럼 기계적으로 규칙적이지 않고,
        #    연속으로 몰리거나 한동안 안 나오는 등 자연스럽게 들쭉날쭉하다.
        idx = int(getattr(self, "_img_rr_idx", 0))
        self._img_rr_idx = idx + 1
        use_ctrl_t = random.random() < 0.25
